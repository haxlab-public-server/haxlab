...
Например: !help bb покажет описание команды bb.`,
    ...
    banauth: {
        ...
        roles: Role.MASTER,
        desc: `
    Эта команда банит игрока по его auth — работает даже если он сейчас не в комнате, и переживает переподключение под тем же auth (в отличие от обычного бана).
    Она принимает от 1 до 2 аргументов:
    Аргумент 1: <auth> auth игрока (посмотреть можно командой '!players' или в чате при входе/выходе).
    Аргумент 2 (опционально): <reason> причина бана.
    Пример: !banauth abc123... читер забанит игрока с этим auth с причиной "читер".`,
