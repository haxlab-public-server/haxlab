...
module.exports = function createMovementEvents({
    ...
    async function onPlayerJoin(player) {
        ...
        // Matched on EITHER auth or conn, not just auth — auth is tied to
        // the HaxBall account and can be reset/regenerated by anyone
        // motivated enough (clearing the browser's own storage, a
        // dedicated tool, etc.), but conn is a fingerprint of the actual
        // network connection and is much harder to spoof, so this catches
        // multi-accounting/ghost-rejoin attempts that only changing auth
        // would otherwise slip past.
        const duplicateCheck = state.playersAll.filter((p) => p.id != player.id && (authArray[p.id][0] == player.auth || authArray[p.id][1] == player.conn));
